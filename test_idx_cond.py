...
msresnet = torch.load('weights_load/weight_epoch_3000.pkl', weights_only=False)
...
with open("predicted_amp.txt", "w", encoding="utf-8") as file:
    file.write('pred (amp) / real (amp) / pred (shift) / real (shift) / idx\n')
...
for i, (samples, amps, shifts, idxs) in enumerate(test_data_loader):
    with torch.no_grad():
        ...
        label_error += loss.item()

    # Print amplitude as txt
    with open("predicted_unnorm_cond.txt", "a", encoding="utf-8") as file:
        file.write(str(predict_amp.item()) + ' / ' + str(amps.item()) + ' / ' + str(predict_shift.item()) + ' / ' + str(
            shifts.item()) + ' / ' + str(idxs.item()) + '\n')
...
print("Total Amplitude & Shift Error:", (label_error / test_size))
print("Total Sample Error:", (sample_error / test_size).item())

with open("predicted_unnorm.txt", "a", encoding="utf-8") as file:
    file.write('\n')
    file.write("Total Amplitude & Shift Error:" + str(label_error / test_size))
